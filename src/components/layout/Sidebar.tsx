...
export function Sidebar({ className = '' }: SidebarProps) {
  ...
  // Animações otimizadas com timing consistente
            ...
            >
              <div className="p-6 h-full flex flex-col">
                ...
                >
                  <div className="flex items-center space-x-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                      <h2 className="font-semibold text-gray-900 text-sm leading-tight truncate">{user?.name || 'Usuário'}</h2>
                      <p className="text-xs text-gray-500 leading-tight">Perfil e configurações</p>
                    </div>
                  </div>
                  <motion.div
                    whileHover={{ scale: 1.02 }}
                ...
                <motion.div 
                  ...
                  className={`mb-6 p-4 ${energyStatus.bgColor} rounded-xl border ${energyStatus.borderColor} shadow-sm`}
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-gray-900 leading-tight">{energyStatus.label}</p>
                      <p className="text-xs text-gray-600 leading-tight mt-0.5">{sidebarEnergyData.remaining} de {sidebarEnergyData.total} disponível</p>
                    </div>
                  </div>
                    ...
                    return (
                      ...
                      >
                        <button
                          ...
                          className={`relative w-full p-4 rounded-xl transition-all duration-200 text-left group h-auto focus:ring-2 focus:ring-blue-500 focus:outline-none ${
                            active 
                        ...
                        >
                          <div className="flex items-center">
                            <div className="w-12 flex justify-center flex-shrink-0">
                              <div className={`p-2.5 rounded-lg transition-all duration-200 ${
                                active 
                                  ...
                                  : 'bg-blue-50 border border-blue-100 group-hover:bg-blue-100 group-hover:border-blue-200 shadow-sm'
                              }`}>
                            ...
                            <div className="flex-1 min-w-0 ml-3">
                              ...
                              <div className={`text-xs leading-tight mt-0.5 transition-colors truncate ${
                                active 
                                  ? 'text-white/80' 
                                  : 'text-gray-500'
                              }`}>
                    ...
                    {utilityItems.map((item) => {
                      ...
                      return (
                        ...
                        >
                          <button
                            onClick={() => handleNavigation(item.path)}
                            className={`w-full flex items-center px-3 py-3 rounded-lg transition-all duration-200 group h-auto justify-start focus:ring-2 focus:ring-blue-500 focus:outline-none ${
                          ...
                          >
                            ...
                            <div className="flex-1 text-left min-w-0 ml-3">
                              <div className="font-medium text-sm text-gray-700 leading-tight">{item.label}</div>
                              <div className="text-xs text-gray-500 leading-tight mt-0.5">{item.description}</div>
                            </div>
                          </button>
                        </motion.div>
                      );
  ...
  const DesktopSidebar = () => (
    ...
    >
        ...
        <div className="flex items-center justify-between">
          <AnimatePresence>
            {!isCollapsed && (
              ...
              >
                <div 
                  className="w-8 h-8 bg-blue-600 rounded-lg overflow-hidden cursor-pointer hover:shadow-md focus:ring-2 focus:ring-blue-500 focus:outline-none transition-all duration-200 flex items-center justify-center flex-shrink-0"
                  onClick={() => handleNavigation('/profile')}
                  role="button"
                  tabIndex={0}
                >
                ...
                <div className="flex-1 text-left min-w-0">
                  <div className="font-semibold text-sm text-gray-900 truncate leading-tight">{user?.name || 'Usuário'}</div>
                  <div className="text-xs text-gray-500 leading-tight">Perfil e configurações</div>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        <AnimatePresence>
          {!isCollapsed && (
            <motion.div 
              ...
              animate="expanded"
              exit="collapsed"
              className={`mt-4 p-3 ${energyStatus.bgColor} rounded-xl border ${energyStatus.borderColor} shadow-sm`}
            >
              <div className="flex items-center space-x-3">
                <div className="flex-shrink-0">
      ...
      <div className={`flex-1 ${isCollapsed ? 'p-3' : 'p-6'} space-y-3 overflow-y-auto`}>
        {navigationItems.map((item, index) => {
          const Icon = item.icon;
          const active = isActive(item.path);
          const isPrimary = item.primary;
                ...
                >
                  ...
                  <AnimatePresence>
                    {!isCollapsed && (
                      ...
                      >
                        <div className="font-medium text-sm text-gray-700 leading-tight">{item.label}</div>
                        <div className="text-xs text-gray-500 truncate leading-tight mt-0.5">{item.description}</div>
                      </motion.div>
                    )}
                  </AnimatePresence>
                </button>
